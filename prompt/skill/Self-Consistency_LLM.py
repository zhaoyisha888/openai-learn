...
#通过设置temperature=0.7来增加模型的随机性，生成多条不同的答案。
def get_completion(prompt, model=modelName, temperature=0.7):
...
# 使用大模型来统计和选择最一致的答案
#TODO: 添加模型选择逻辑
def get_most_common_answer_via_model(responses, model=modelName):
    responses_str = "\n".join(responses)
    prompt_for_model = f"""请根据以下生成的多个答案：{responses_str}，\n选择出现次数最多的答案作为最终答案。最终答案是："""
    final_answer = get_completion(prompt_for_model,model,0).strip()
    return final_answer
